  ...
  <div>
    <section class="wrapper">
      ...
      <ul class="featured-items">
        <li v-for="product in filteredList" :key="product[1].id" class="productList">
          <img :src="product[1].picture" height=200px><br>
          <p class="product-title">{{ product[1].name }}</p>
        <p><i>${{ product[1].price }}</i></p>
        <p><i>Description: {{ product[1].description }}</i></p>
        <button class="product-title" v-bind:id="product[0]" v-on:click.prevent=route($event)>More Info</button>
        </li>
      </ul>
